    ...
    pub fn gen_mirs(&mut self, mirs: &[Mir]) {
        for mir in mirs {
            if let Some(ref name) = mir.name {
                ...
                let param_types: Vec<BasicMetadataTypeEnum<'ctx>> = mir.locals.keys().map(|_| BasicMetadataTypeEnum::from(BasicTypeEnum::Int(self.i64_type))).collect();
    ...
    fn get_callee(&mut self, func: &str) -> inkwell::values::FunctionValue<'ctx> {
        if let Some(f) = self.fns.get(func) {
            *f
            ...
            let param_types: Vec<BasicMetadataTypeEnum<'ctx>> = vec![BasicMetadataTypeEnum::from(BasicTypeEnum::Int(self.i64_type))];
