...
impl<'ctx> LLVMCodegen<'ctx> {
    ...
    pub fn gen_intrinsics(&mut self) {
        ...
        let tbaa_id = self
            .context
            .metadata_value(self.i32_type.const_int(0, false));
        let tbaa_node = self
            .context
            .metadata_value(self.i32_type.const_int(1, false));
        ...
        let vec_hint = self
            .context
            .metadata_value(self.i32_type.const_int(4, false));
        ...
        let len = self
            .builder
            .build_load(len_ptr, "load_len")
            .into_int_value();
        let new_len = self
            .builder
            .build_int_add(len, self.i32_type.const_int(1, false), "new_len");
        let new_size =
            self.builder
                .build_int_mul(new_len, self.i64_type.const_int(4, false), "new_size");
        ...
        let new_ptr = self
            .builder
            .build_call(malloc_val, &[new_size.into()], "new_ptr")
            .try_as_basic_value()
            .left()
            .unwrap()
            .into_pointer_value();
        let old_ptr = self
            .builder
            .build_struct_gep(vec_arg, 0, "old_ptr")
            .unwrap();
        let old_data = self
            .builder
            .build_load(old_ptr, "old_data")
            .into_pointer_value();
        let memcpy_type = self.i8ptr_type.fn_type(
            &[
        ...
        let memcpy = self
            .module
            .add_function("llvm.memcpy.p0i8.p0i8.i64", memcpy_type, None);
        let size_bytes =
            self.builder
                .build_int_mul(len, self.i64_type.const_int(4, false), "size_bytes");
        self.builder.build_call(
            memcpy,
            &[new_ptr.into(), old_data.into(), size_bytes.into()],
            "memcpy_old",
        );
        // SIMD add (v4i32)
        ...
        let data_ptr = self.builder.build_pointer_cast(
            new_ptr,
            self.i32_type.ptr_type(inkwell::AddressSpace::Generic),
            "data_cast",
        );
        let simd_load = self
            .builder
            .build_load(data_ptr, "simd_load")
            .into_vector_value();
        let broadcast = self
            .builder
            .build_int_nsw_vector_splat(vec_ty.len(), scalar, "broadcast");
        ...
        let new_vec_ptr = self
            .builder
            .build_pointer_cast(new_ptr, vec_ptr_type, "new_vec_cast");
        let new_len_ptr = self
            .builder
            .build_struct_gep(new_vec_ptr, 1, "new_len_ptr")
            .unwrap();
        ...
        let send_type = self
            .i32_type
            .fn_type(&[self.i8ptr_type.into(), self.i32_type.into()], false);
        ...
        let http_val = self
            .module
            .add_function("std_net_http_get", http_type, None);
        ...
        let serde_type = self
            .i8ptr_type
            .fn_type(&[self.i8ptr_type.into(), self.i8ptr_type.into()], false);
        let serde_val = self
            .module
            .add_function("std_serde_json_parse", serde_type, None);
        ...
        let gov_val = self
            .module
            .add_function("std_governor_permit", gov_type, None);
        ...
        let prom_val = self
            .module
            .add_function("std_prometheus_inc", prom_type, None);
        self.prometheus_inc_fn = Some(prom_val);

        // copy (for Copy derive: memcpy)
        let copy_type = self.i8ptr_type.fn_type(
            &[
                self.i8ptr_type.into(),
                self.i8ptr_type.into(),
                self.i64_type.into(),
            ...
            AstNode::Call {
                ...
                if let Some(f) = self.module.get_function(&func_name) {
                    ...
                    if let (Some(x), Some(y)) =
                        (arg_vals[0].into_int_value(), arg_vals[1].into_int_value())
                    {
    ...
    pub fn gen_func(
        ...
        ast: &AstNode,
        resolver: &Resolver,
        param_map: &mut HashMap<String, u32>,
    ) {
    ...
    pub fn gen_actor(&mut self, ast: &AstNode) {
        if let AstNode::ActorDef { name, methods } = ast {
            ...
            let handler = self
                .module
                .add_function(&format!("{name}_handler"), handler_type, None);
            ...
            let inc_type = self
                .i32_type
                .fn_type(&[actor_ptr_type.into(), self.i32_type.into()], false);
