    ...
    fn gen_stmt(&mut self, stmt: &MirStmt) {
        match stmt {
            ...
            MirStmt::SemiringFold { op, values, result } => {
                let mut acc: BasicValueEnum<'ctx> = self.load_local(values[0]);
                ...
                if values.len() >= 4 && self.enable_simd {
                    for chunk in values[1..].chunks(4) {
                        ...
                        acc = match op {
                            SemiringOp::Add => self.builder.build_int_add(acc.into_vector_value().unwrap(), vec, "fold_add_simd").unwrap().into(),
                            SemiringOp::Mul => self.builder.build_int_mul(acc.into_vector_value().unwrap(), vec, "fold_mul_simd").unwrap().into(),
                        };
                    }
                } else {
                    for &v in &values[1..] {
