...
use crate::blockchain::common::types::{Amount, Network, TransactionId};
...
pub fn Solana_Transaction_new(
    ...
    log::debug!(
        "Creating new Solana transaction with {} instructions",
        instructions.len()
    );
...
pub fn Solana_Transaction_sign(
    ...
    if private_key.len() != 64 {
        return Err(BlockchainError::crypto(format!(
            "Invalid private key length: {} bytes (expected 64 for Ed25519)",
            private_key.len()
        )));
    }
...
pub fn Solana_Transaction_deserialize(
    bytes: &[u8],
    network: Network,
) -> Result<SolanaTransaction, BlockchainError> {
    if bytes.len() < 1 {
...
pub fn Solana_Transaction_create_transfer_instruction(
    ...
    if from_pubkey.len() != 32 {
        return Err(BlockchainError::transaction(format!(
            "Invalid from pubkey length: {} bytes (expected 32)",
            from_pubkey.len()
        )));
    }
    ...
    if to_pubkey.len() != 32 {
        return Err(BlockchainError::transaction(format!(
            "Invalid to pubkey length: {} bytes (expected 32)",
            to_pubkey.len()
        )));
    }
    ...
    Ok(TransactionInstruction {
        ...
        accounts: vec![from_pubkey.to_vec(), to_pubkey.to_vec()],
