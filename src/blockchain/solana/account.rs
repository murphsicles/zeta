...
use crate::blockchain::common::types::{Address, Amount};
...
pub fn Solana_Account_get(address: &Address) -> Result<SolanaAccount, BlockchainError> {
    ...
    if pubkey.len() != 32 {
        return Err(BlockchainError::address(format!(
            "Invalid pubkey length: {} bytes (expected 32)",
            pubkey.len()
        )));
    }
...
pub fn Solana_Account_create(
    ...
    if pubkey.len() != 32 {
        return Err(BlockchainError::account(format!(
            "Invalid pubkey length: {} bytes (expected 32)",
            pubkey.len()
        )));
    }
    ...
    if owner.len() != 32 {
        return Err(BlockchainError::account(format!(
            "Invalid owner length: {} bytes (expected 32)",
            owner.len()
        )));
    }
...
pub fn Solana_Account_deserialize(bytes: &[u8]) -> Result<SolanaAccount, BlockchainError> {
    ...
    if bytes.len() < min_len {
        return Err(BlockchainError::account(format!(
            "Account data too short: {} bytes (minimum {})",
            bytes.len(),
            min_len
        )));
    }
    ...
    Ok(SolanaAccount::new(
        pubkey, lamports, owner, data, executable, rent_epoch,
    ))
}
