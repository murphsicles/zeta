...
pub fn Bitcoin_Address_from_pubkey(
    ...
    if pubkey.len() != 33 && pubkey.len() != 65 {
        return Err(BlockchainError::address(format!(
            "Invalid public key length: {} bytes (expected 33 or 65)",
            pubkey.len()
        )));
    }
    ...
    let nour_network = match network.to_lowercase().as_str() {
        ...
        _ => {
            return Err(BlockchainError::address(format!(
                "Unknown network: {} (expected 'mainnet', 'testnet', or 'stn')",
                network
            )));
        }
    };
    ...
    let nour_addr = NourAddress::from_pubkey_hash(nour::util::hash160(pubkey), nour_network);
