...
pub fn Bitcoin_Key_from_private(
    private_key: &[u8],
    compressed: bool,
) -> Result<BtcKeyPair, BlockchainError> {
    if private_key.len() != 32 {
        return Err(BlockchainError::key(format!(
...
pub fn Bitcoin_Key_sign(message: &[u8], private_key: &[u8]) -> Result<Vec<u8>, BlockchainError> {
    if message.len() != 32 {
        return Err(BlockchainError::crypto(format!(
            "Invalid message length: {} bytes (expected 32)",
            message.len()
        )));
    }
...
pub fn Bitcoin_Key_verify(
    ...
    Ok(secp
        .verify_ecdsa(&message_obj, &signature_obj, &public_key_obj)
        .is_ok())
}
...
pub fn Bitcoin_Key_sign_tx_hash(
    ...
    if tx_hash.len() != 32 {
        return Err(BlockchainError::crypto(format!(
            "Invalid transaction hash length: {} bytes (expected 32)",
            tx_hash.len()
        )));
    }
...
pub fn Bitcoin_Key_verify_tx_signature(
    ...
    if signature.len() != 65 {
        return Err(BlockchainError::crypto(format!(
            "Invalid signature length: {} bytes (expected 65)",
            signature.len()
        )));
    }
