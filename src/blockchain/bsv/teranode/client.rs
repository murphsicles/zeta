...
impl TeranodeClient {
    ...
    pub fn new(config: TeranodeClientConfig) -> Result<Self, TeranodeError> {
        config.validate().map_err(|e| TeranodeError::Config(e))?;
    ...
    pub fn from_env() -> Result<Self, TeranodeError> {
        let config = TeranodeClientConfig::from_env().map_err(|e| TeranodeError::Config(e))?;
        Self::new(config)
    }
            ...
            match mining_info_result {
                Ok(info) => {
                    ...
                    let hash_rate = info
                        ...
                        .and_then(|v| v.as_f64())
                        .unwrap_or(0.0);
                    let total_hashes = info
                        .get("totalhashes")
                        .and_then(|v| v.as_u64())
                        .unwrap_or(0);
                    let accepted_shares =
                        info.get("accepted").and_then(|v| v.as_u64()).unwrap_or(0);
                    let rejected_shares =
                        info.get("rejected").and_then(|v| v.as_u64()).unwrap_or(0);
                    (
                        is_mining,
                        hash_rate,
                        total_hashes,
    ...
    pub async fn estimate_fee_rate(&self, blocks: u32) -> Result<f64, TeranodeError> {
        ...
        result
            ...
            .and_then(|v| v.as_f64())
            .ok_or_else(|| {
                TeranodeError::InvalidResponse("Missing feerate in response".to_string())
            })
    }
