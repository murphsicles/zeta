...
impl IdentityAwareBorrowChecker {
    ...
    pub fn use_variable(
        &mut self,
        name: &str,
        required_capabilities: &[CapabilityLevel],
    ) -> Result<(), String> {
        if let Some(var_info) = self.variables.get_mut(name) {
            if !var_info.can_use() {
                return Err(format!(
                    "Cannot use variable '{}': it has been moved or is mutably borrowed",
                    name
                ));
            }
            ...
            if !var_info.has_capabilities(required_capabilities) {
                return Err(format!(
                    "Insufficient capabilities to use variable '{}'. Required: {:?}, Available: {:?}",
    ...
    pub fn move_variable(
        ...
        if let Some(var_info) = self.variables.get_mut(name) {
            if !var_info.can_move() {
                return Err(format!(
                    "Cannot move variable '{}': it is already borrowed or moved",
                    name
                ));
            }
            ...
            if !var_info.has_capabilities(required_capabilities) {
                return Err(format!(
                    "Insufficient capabilities to move variable '{}'. Required: {:?}, Available: {:?}",
                    name, required_capabilities, var_info.required_capabilities
                ));
            }
    ...
    pub fn borrow_immutable(
        ...
        if let Some(var_info) = self.variables.get_mut(name) {
            if !var_info.can_borrow_immutably() {
                return Err(format!(
                    "Cannot immutably borrow variable '{}': it is mutably borrowed or moved",
                    name
                ));
            }
            ...
            if !var_info.has_capabilities(required_capabilities) {
                return Err(format!(
                    "Insufficient capabilities to immutably borrow variable '{}'. Required: {:?}, Available: {:?}",
                    name, required_capabilities, var_info.required_capabilities
                ));
            }
    ...
    pub fn borrow_mutable(
        ...
        if let Some(var_info) = self.variables.get_mut(name) {
            if !var_info.can_borrow_mutably() {
                return Err(format!(
                    "Cannot mutably borrow variable '{}': it is already borrowed",
                    name
                ));
            }
            ...
            if !var_info.has_capabilities(required_capabilities) {
                return Err(format!(
                    "Insufficient capabilities to mutably borrow variable '{}'. Required: {:?}, Available: {:?}",
                    name, required_capabilities, var_info.required_capabilities
                ));
            }
    ...
    pub fn release_immutable_borrow(&mut self, name: &str) -> Result<(), String> {
        if let Some(var_info) = self.variables.get_mut(name) {
            if var_info.immutable_borrow_count == 0 {
                return Err(format!(
                    "No immutable borrow to release for variable '{}'",
                    name
                ));
            }
    ...
    pub fn release_mutable_borrow(&mut self, name: &str) -> Result<(), String> {
        if let Some(var_info) = self.variables.get_mut(name) {
            if !var_info.mutable_borrow_exists {
                return Err(format!(
                    "No mutable borrow to release for variable '{}'",
                    name
                ));
            }
