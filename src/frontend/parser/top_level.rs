...
//! Parsers for Zeta top-level definitions.
//! Includes functions, concepts, implementations, enums, and structs.
...
fn parse_method_sig(input: &str) -> IResult<&str, AstNode> {
    ...
    let generics = generics_opt.unwrap_or_default();
    let ret = ret_opt.unwrap_or_else(|| "i64".to_string());
    Ok((input, AstNode::Method { name, params, ret, generics, doc: "".to_string() }))
}
...
fn parse_concept(input: &str) -> IResult<&str, AstNode> {
    ...
    Ok((input, AstNode::ConceptDef { name, methods: body, doc: "".to_string() }))
}
...
fn parse_impl(input: &str) -> IResult<&str, AstNode> {
    ...
    let (input, _) = parse_keyword("for")(input)?;
...
fn parse_struct(input: &str) -> IResult<&str, AstNode> {
    ...
    let (input, name) = ws(parse_ident).parse(input)?;
    let (input, fields) = delimited(ws(tag("{")), many0(map(pair(ws(parse_ident), preceded(ws(tag(":")), ws(parse_ident))), |(n, t)| (n, t))), ws(tag("}"))).parse(input)?;
