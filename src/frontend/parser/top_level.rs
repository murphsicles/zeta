...
fn parse_const(input: &str) -> IResult<&str, AstNode> {
    println!("[PARSER DEBUG] parse_const called, input: {:?}", &input[..50.min(input.len())]);
    let (input, _) = ws(tag("const")).parse(input)?;
    let (input, name) = ws(parse_ident).parse(input)?;
    let (input, _) = ws(tag(":")).parse(input)?;
    let (input, ty) = ws(parse_type).parse(input)?;
    let (input, _) = ws(tag("=")).parse(input)?;
    let (input, value) = ws(super::expr::parse_full_expr).parse(input)?;
    let (input, _) = ws(tag(";")).parse(input)?;
    
    println!("[PARSER DEBUG] parse_const succeeded: {}: {} = ...", name, ty);
    Ok((
        input,
        AstNode::ConstDef {
            name,
...
fn parse_top_level_item(input: &str) -> IResult<&str, AstNode> {
    alt((
        parse_const,    // Try const first (before func, since 'const' could be misparsed as start of 'extern fn')
        parse_func,
        parse_type_alias,
        parse_concept,
        parse_impl,
