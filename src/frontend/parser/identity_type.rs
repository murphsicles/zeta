...
pub fn parse_identifier(input: &str) -> IResult<&str, String> {
    let ident = recognize(pair(alt((alpha1, tag("_"))), opt(alphanumeric1)));
...
pub fn parse_constraint(input: &str) -> IResult<&str, IdentityConstraint> {
    ...
    let capability_constraint = map(parse_capability, |cap_str: String| {
    ...
    let length_constraint = map(
        (tag("length"), ws(alt((tag(">="), tag("<=")))), ws(digit1)),
    ...
    let pattern_constraint = map(
        (tag("matches"), ws(tag("'")), alphanumeric1, ws(tag("'"))),
        |(_, _, pattern, _): (&str, &str, &str, &str)| {
            IdentityConstraint::Pattern(pattern.to_string())
        },
    );

    alt((capability_constraint, length_constraint, pattern_constraint)).parse(input)
}
...
pub fn parse_type_param(input: &str) -> IResult<&str, (String, Option<IdentityConstraint>)> {
    ...
    let (input, constraint_opt) = opt(preceded(ws(tag(":")), parse_constraint)).parse(input)?;
...
pub fn parse_parametric_identity(
    input: &str,
) -> IResult<&str, (String, Vec<(String, Option<IdentityConstraint>)>)> {
...
pub fn parse_identity_annotation(input: &str) -> IResult<&str, Vec<String>> {
    delimited(ws(tag("[identity:")), parse_capability_list, ws(tag("]"))).parse(input)
}
...
mod tests {
    ...
    fn test_parse_capability() {
        ...
        assert_eq!(
            parse_capability("immutable"),
            Ok(("", "immutable".to_string()))
        );
    }
    ...
    fn test_parse_constraint() {
        ...
        assert_eq!(
            parse_constraint("read"),
            Ok(("", IdentityConstraint::Capability(CapabilityLevel::Read)))
        );
        assert_eq!(
            parse_constraint("write"),
            Ok(("", IdentityConstraint::Capability(CapabilityLevel::Write)))
        );
        assert_eq!(
            parse_constraint("execute"),
            Ok(("", IdentityConstraint::Capability(CapabilityLevel::Execute)))
        );
        assert_eq!(
            parse_constraint("owned"),
            Ok(("", IdentityConstraint::Capability(CapabilityLevel::Owned)))
        );
        assert_eq!(
            parse_constraint("immutable"),
            Ok((
                "",
                IdentityConstraint::Capability(CapabilityLevel::Immutable)
            ))
        );
        assert_eq!(
            parse_constraint("length >= 5"),
            Ok(("", IdentityConstraint::MinLength(5)))
        );
        assert_eq!(
            parse_constraint("length <= 10"),
            Ok(("", IdentityConstraint::MaxLength(10)))
        );
        assert_eq!(
            parse_constraint("matches 'pattern'"),
            Ok(("", IdentityConstraint::Pattern("pattern".to_string())))
        );
    }
    ...
    fn test_parse_type_param() {
        ...
        assert_eq!(
            parse_type_param("T: read"),
            Ok((
                "",
                (
                    "T".to_string(),
                    Some(IdentityConstraint::Capability(CapabilityLevel::Read))
                )
            ))
        );
        assert_eq!(
            parse_type_param("T: length >= 5"),
            Ok((
                "",
                ("T".to_string(), Some(IdentityConstraint::MinLength(5)))
            ))
        );
    }
    ...
    fn test_parse_type_params() {
        ...
        assert_eq!(
            parse_type_params("<T>"),
            Ok(("", vec![("T".to_string(), None)]))
        );
        assert_eq!(
            parse_type_params("<T, U>"),
            Ok(("", vec![("T".to_string(), None), ("U".to_string(), None)]))
        );
        assert_eq!(
            parse_type_params("<T: read, U: write>"),
            Ok((
                "",
                vec![
                    (
                    ...
                    (
                        "U".to_string(),
                        Some(IdentityConstraint::Capability(CapabilityLevel::Write))
                    )
                ]
            ))
        );
    ...
    fn test_parse_parametric_identity() {
        ...
        assert_eq!(
            parse_parametric_identity("Identity<T>"),
            Ok(("", ("Identity".to_string(), vec![("T".to_string(), None)])))
        );
        assert_eq!(
            parse_parametric_identity("Identity<T: read>"),
            Ok((
                "",
                (
                    "Identity".to_string(),
        ...
        assert_eq!(
            parse_parametric_identity("Identity<T: read, U: write>"),
            Ok((
                "",
                (
                    "Identity".to_string(),
