...
impl ClusterManager {
    ...
    pub async fn start(self: Arc<Self>) -> tokio::task::JoinHandle<()> {
        ...
        let _ = self
            .event_sender
            .send(MembershipEvent::NodeJoined(self.local_node.clone()));
    ...
    pub fn stop(&self) {
        ...
        *running = false;
        ...
        let _ = self
            .event_sender
            .send(MembershipEvent::NodeLeft(self.local_node.id));
    }
    ...
    async fn update_node_status(&self, node_id: NodeId, status: NodeStatus) {
        ...
        if old_status != status {
            let _ = self
                .event_sender
                .send(MembershipEvent::NodeStatusChanged(node_id, status));
        }
    }
    ...
    pub async fn add_node(&self, node_info: NodeInfo) -> Result<(), String> {
        ...
        let _ = self
            .event_sender
            .send(MembershipEvent::NodeJoined(node_info));
        Ok(())
    }
