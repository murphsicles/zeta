...
pub unsafe extern "C" fn runtime_calloc(num: usize, size: usize) -> i64 {
    ...
    let ptr = unsafe { runtime_malloc(total_size) };
...
pub unsafe extern "C" fn test_bulletproof_features() -> i64 {
    ...
    let ptr = runtime_malloc(100);
    if ptr == 0 {
        return -1;
    }
    
    // Test bounds checking (should succeed)
    ...
    // Test double-free detection (should report corruption but not crash)
    runtime_free(ptr);
