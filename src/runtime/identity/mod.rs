...
pub use bridge::{
    DefaultRuntimeHooks, GlobalIdentityRuntime, IdentityRuntimeManager, RuntimeIdentityHooks,
};
pub use integration::{
    identity_host_str_concat, identity_host_str_contains, identity_host_str_ends_with,
    identity_host_str_len, identity_host_str_replace, identity_host_str_starts_with,
    identity_host_str_to_lowercase, identity_host_str_to_uppercase, identity_host_str_trim,
    init_global_identity_context, reset_global_identity_context,
};
pub use validation::{
    CapabilityValidator, DefaultValidationHook, IdentityContext, IdentityValidationHook,
    ValidationError,
};
...
pub mod ops {
    ...
    pub fn create_identity_context(
        ...
        let identity = IdentityType::with_value(identity_str.to_string(), vec![capability]);
...
mod tests {
    ...
    fn test_validate_string_operation() {
        ...
        assert!(
            ops::validate_string_operation(&read_context, "concat", "hello", &["world"]).is_ok()
        );
        assert!(
            ops::validate_string_operation(&read_context, "trim_start", "  hello", &[]).is_err()
        );
    }
