...
impl Actor for EchoActor {
    ...
    fn on_stop(&mut self, context: ActorContext) {
        println!(
            "EchoActor {} stopped after {} messages",
            context.id().0,
            self.received
        );
    }
...
impl Actor for WorkerActor {
    ...
    fn on_message(&mut self, context: ActorContext, message: ActorMessage) {
        ...
        match message {
            ...
            _ => {
                println!(
                    "WorkerActor {} received unsupported message type",
                    context.id().0
                );
            }
        }
    ...
    fn on_stop(&mut self, context: ActorContext) {
        println!(
            "WorkerActor {} stopped after processing {} tasks",
            context.id().0,
            self.processed
        );
    }
...
mod tests {
    ...
    async fn test_actor_system() {
        ...
        // Send messages
        echo_handle
            .send(ActorMessage::Text("Hello".to_string()))
            .unwrap();
        worker_handle.send(ActorMessage::Number(42)).unwrap();
        worker_handle
            .send(ActorMessage::Text("test".to_string()))
            .unwrap();
