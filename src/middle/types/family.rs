...
impl TypeFamilyPattern {
    ...
    fn match_type_with_subst(&self, ty: &Type, substitution: &mut HashMap<String, Type>) -> bool {
        match (self, ty) {
            ...
            (TypeFamilyPattern::Literal(pattern_ty), _) => pattern_ty == ty,
...
impl TypeFamilyContext {
    ...
    pub fn reduce(&mut self, family_name: &str, args: &[Type]) -> Result<Type, String> {
        ...
        for equation in &family.equations {
            ...
            if let Some(substitution) = equation
                .pattern
                .match_type(&Self::make_application(family_name, args))
            {
                // Check constraints
    ...
    fn make_application(family_name: &str, args: &[Type]) -> Type {
        if args.is_empty() {
            ...
            Type::Named(family_name.to_string(), args.to_vec())
        }
    }
    ...
    fn apply_substitution(&self, ty: &Type, substitution: &HashMap<String, Type>) -> Type {
        match ty {
            ...
            Type::Ref(inner, lifetime, mutability) => Type::Ref(
                Box::new(self.apply_substitution(inner, substitution)),
                lifetime.clone(),
                *mutability,
            ),

            Type::Vector(inner, size) => Type::Vector(
                Box::new(self.apply_substitution(inner, substitution)),
                size.clone(),
            ),
        ...
        match ty {
            Type::Named(name, args) => {
                ...
                if self.families.contains_key(name) {
                    ...
                    let simplified_args: Result<Vec<Type>, String> =
            ...
            Type::Slice(inner) => Ok(Type::Slice(Box::new(self.simplify(inner)?))),

            Type::Tuple(types) => {
                let simplified: Result<Vec<Type>, String> =
                    types.iter().map(|t| self.simplify(t)).collect();
                Ok(Type::Tuple(simplified?))
            }

            Type::Ref(inner, lifetime, mutability) => Ok(Type::Ref(
            ...
            Type::Function(params, ret) => {
                let simplified_params: Result<Vec<Type>, String> =
                    params.iter().map(|p| self.simplify(p)).collect();
