...
impl Substitution {
    ...
    pub fn apply(&self, ty: &Type) -> Type {
        match ty {
            ...
            Type::Slice(inner) =>
                Type::Slice(Box::new(self.apply(inner))),
            Type::Tuple(types) =>
                Type::Tuple(types.iter().map(|t| self.apply(t)).collect()),
            Type::Ptr(inner) =>
                Type::Ptr(Box::new(self.apply(inner))),
            Type::Ref(inner, mutability) =>
                Type::Ref(Box::new(self.apply(inner)), *mutability),
            Type::Named(name, args) =>
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            UnifyError::Mismatch(t1, t2) => 
                write!(f, "Type mismatch: {} vs {}", t1.display_name(), t2.display_name()),
            UnifyError::OccursCheck(var, ty) => 
                write!(f, "Occurs check failed: T{} occurs in {}", var.0, ty.display_name()),
            UnifyError::ArityMismatch(expected, actual) =>
                write!(f, "Arity mismatch: expected {} arguments, got {}", expected, actual),
        }
    }
}
