...
fn test_complex_types() {
    ...
    println!("2. Result<Vec<String>, Box<dyn Error>>: {}", complex.display_name());
    
    // Test 3: Tuple in generic
    let tuple_in_generic = Type::from_string("HashMap<(String, i32), bool>");
    println!("3. HashMap<(String, i32), bool>: {}", tuple_in_generic.display_name());
    ...
    println!("7. Box<dyn std::error::Error + Send + Sync>: {}", box_trait.display_name());
    
    // Test 7: Complex real-world example from v0.5.0
    let real_world = Type::from_string("HashMap<(String, String), Vec<AstNode>>");
    println!("8. HashMap<(String, String), Vec<AstNode>>: {}", real_world.display_name());
