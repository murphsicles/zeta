    ...
    match parse_zeta(code) {
        Ok((remaining, asts)) => {
            ...
            if has_generics {
                ...
                let generic_funcs: Vec<&str> = asts.iter()
                    .filter_map(|ast| {
                ...
                if !generic_funcs.is_empty() {
                    ...
                    if has_generic_calls {
                        ...
                        let issue = "Generic functions found but no generic calls detected".to_string();
                        println!("  ⚠️  {}", issue);
                        issues.push(issue);
                    }
                }
...
fn main() {
    ...
    println!("\nOverall: {}/{} tests passed", total_passed, test_results.len());
