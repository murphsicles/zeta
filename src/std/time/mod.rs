...
pub fn register_functions(map: &mut std::collections::HashMap<&'static str, usize>) {
    ...
    map.insert(
        ...
        system_time_since_epoch as *const () as usize,
    );
    ...
    map.insert(
        "duration_from_secs",
        duration_from_secs as *const () as usize,
    );
    map.insert(
        "duration_from_millis",
        duration_from_millis as *const () as usize,
    );
    map.insert(
        "duration_from_micros",
        duration_from_micros as *const () as usize,
    );
    map.insert(
        "duration_from_nanos",
        duration_from_nanos as *const () as usize,
    );
    map.insert("duration_as_secs", duration_as_secs as *const () as usize);
    map.insert(
        "duration_as_millis",
        duration_as_millis as *const () as usize,
    );
    map.insert(
        "duration_as_micros",
        duration_as_micros as *const () as usize,
    );
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn system_time_since_epoch(system_time: *const ZetaSystemTime) -> u128 {
    unsafe {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn instant_elapsed(instant: *const ZetaInstant) -> u128 {
    unsafe {
...
/// duration must be a valid pointer from duration_from_*.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn duration_as_millis(duration: *const ZetaDuration) -> u128 {
    unsafe {
        if let Some(duration) = duration.as_ref() {
...
/// duration must be a valid pointer from duration_from_*.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn duration_as_micros(duration: *const ZetaDuration) -> u128 {
    unsafe {
        if let Some(duration) = duration.as_ref() {
...
/// duration must be a valid pointer from duration_from_*.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn duration_as_nanos(duration: *const ZetaDuration) -> u128 {
    unsafe {
        if let Some(duration) = duration.as_ref() {
...
pub unsafe extern "C" fn unix_epoch() -> *mut ZetaSystemTime {
    let epoch = Box::new(ZetaSystemTime { inner: UNIX_EPOCH });
    Box::into_raw(epoch)
}
