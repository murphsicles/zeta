...
impl QuantumGate {
    ...
    fn apply_toffoli(
        ...
        if state.qubits >= 3
            && control1 < state.qubits
            && control2 < state.qubits
            && target < state.qubits
            && control1 != control2
            && control1 != target
            && control2 != target
...
impl QuantumCircuit {
    ...
    pub fn toffoli(&mut self, control1: usize, control2: usize, target: usize) -> &mut Self {
        self.add_gate(QuantumGate::Toffoli {
            control1,
            control2,
            target,
        });
...
pub fn register_functions(map: &mut std::collections::HashMap<&'static str, usize>) {
    ...
    map.insert(
        "quantum_circuit_new",
        quantum_circuit_new as *const () as usize,
    );
    map.insert("quantum_circuit_h", quantum_circuit_h as *const () as usize);
    map.insert(
        "quantum_circuit_cnot",
        quantum_circuit_cnot as *const () as usize,
    );
    map.insert(
        "quantum_circuit_measure",
        quantum_circuit_measure as *const () as usize,
    );
    map.insert(
        "quantum_circuit_execute",
        quantum_circuit_execute as *const () as usize,
    );

    // Quantum algorithms
    map.insert(
        "shors_algorithm_new",
        shors_algorithm_new as *const () as usize,
    );
    map.insert(
        "shors_algorithm_factor",
        shors_algorithm_factor as *const () as usize,
    );
    map.insert(
        "grovers_algorithm_new",
        grovers_algorithm_new as *const () as usize,
    );
    map.insert(
        "grovers_algorithm_search",
        grovers_algorithm_search as *const () as usize,
    );
}
...
#[unsafe(no_mangle)]
pub extern "C" fn quantum_circuit_cnot(
...
#[unsafe(no_mangle)]
pub extern "C" fn quantum_circuit_execute(
    circuit: *const QuantumCircuit,
) -> *mut (QuantumState, Vec<bool>) {
    if circuit.is_null() {
...
#[unsafe(no_mangle)]
pub extern "C" fn grovers_algorithm_search(
