...
impl Conv2d {
    ...
    fn conv2d_forward(&self, input: &Tensor) -> Tensor {
        ...
        assert_eq!(
            input.ndim(),
            4,
            "Input must be 4D for Conv2d: [batch, channels, height, width]"
        );
        ...
        let output = Tensor::zeros(
            output_shape,
            input.requires_grad() || self.weights.requires_grad(),
        );
...
impl Layer for RNN {
    fn forward(&self, input: &Tensor) -> Tensor {
        ...
        assert_eq!(
            input.ndim(),
            3,
            "RNN input must be 3D: [seq_len, batch, features]"
        );
...
impl Layer for LSTM {
    fn forward(&self, input: &Tensor) -> Tensor {
        ...
        assert_eq!(
            input.ndim(),
            3,
            "LSTM input must be 3D: [seq_len, batch, features]"
        );
