...
impl LossFunction for MSELoss {
    ...
    fn gradient(&self, predictions: &Tensor, targets: &Tensor) -> Tensor {
        assert_eq!(
            predictions.shape(),
            targets.shape(),
            "Shape mismatch for MSE gradient"
        );
...
impl Trainer {
    ...
    pub fn validate(&self, val_loader: &mut DataLoader) -> (f32, f32) {
        ...
        let val_loss = if batch_count > 0 {
            total_loss / batch_count as f32
        } else {
            0.0
        };
        let val_accuracy = if total_samples > 0 {
            total_correct as f32 / total_samples as f32
        } else {
            0.0
        };
    ...
    pub fn train(
        ...
        for epoch in 1..=epochs {
            let (train_loss, val_loss, train_acc, val_acc) =
                self.train_epoch(train_loader, &mut val_loader);

            println!(
                "Epoch {}/{}: train_loss={:.4}, val_loss={:.4}, train_acc={:.4}, val_acc={:.4}, lr={:.6}",
                epoch,
                epochs,
                train_loss,
                val_loss,
