    ...
    pub fn prune(&self, tensor: &Tensor) -> Tensor {
        ...
        match self.method {
            PruningMethod::Magnitude => {
                ...
                let mut magnitudes: Vec<(usize, f32)> = data
                    .iter()
                    .enumerate()
                    .map(|(i, &x)| (i, x.abs()))
                    .collect();
...
impl ModelServer {
    ...
    fn update_metrics(&mut self, inference_time: Duration) {
        ...
        self.metrics.peak_memory_usage =
            self.metrics.peak_memory_usage.max(cache_stats.memory_usage);
    }
...
impl std::fmt::Display for ServerError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ServerError::BatchTooLarge(actual, max) => {
                write!(f, "Batch size {} exceeds maximum {}", actual, max)
            }
...
impl ModelConverter {
    ...
    pub fn convert(&self, model: &dyn Layer) -> Result<(), String> {
        ...
        match self.target_format {
            ModelFormat::ONNX => {
                println!("Converting to ONNX format (opset {})", self.opset_version)
            }
