...
impl MemoryDataset {
    ...
    pub fn train_test_split(
        ...
        let train_features: Vec<Tensor> = train_indices
            .iter()
            .map(|&i| self.features[i].clone())
            .collect();
        let train_labels: Vec<Tensor> = train_indices
            .iter()
            .map(|&i| self.labels[i].clone())
            .collect();

        let test_features: Vec<Tensor> = test_indices
            .iter()
            .map(|&i| self.features[i].clone())
            .collect();
        let test_labels: Vec<Tensor> = test_indices
            .iter()
            .map(|&i| self.labels[i].clone())
            .collect();
...
impl Preprocessor {
    ...
    pub fn standardize(&self, data: &Tensor) -> Tensor {
        ...
        let standardized_data: Vec<f32> = data
            .data()
            .iter()
            .zip(mean)
            .zip(std)
    ...
    pub fn normalize_range(&self, data: &Tensor, range: (f32, f32)) -> Tensor {
        ...
        let scaled_data: Vec<f32> = normalized
            .data()
            .iter()
...
impl CSVDataset {
    ...
    pub fn from_file<P: AsRef<Path>>(
        ...
        Ok(CSVDataset {
            ...
            labels,
            feature_names,
            label_name: if label_column.is_some() {
                "label".to_string()
            } else {
                "".to_string()
            },
    ...
    pub fn to_memory_dataset(&self, name: &str) -> MemoryDataset {
        let features: Vec<Tensor> = self
            .data
            .iter()
            .map(|row| Tensor::new(row.clone(), vec![row.len()], false))
            .collect();

        let labels: Vec<Tensor> = self
            .labels
            .iter()
