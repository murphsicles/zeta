...
fn ident<'a>() -> impl Parser<&'a str, String, nom::error::Error<&'a str>> {
    let first = satisfy(|c| c.is_alphabetic() || c == '_');
    let rest = many0(satisfy(|c| c.is_alphanumeric() || c == '_'));
    first.and(rest).map(|(c, rest)| std::iter::once(c).chain(rest).collect())
}

fn literal<'a>() -> impl Parser<&'a str, AstNode, nom::error::Error<&'a str>> {
    map(nom_i64, AstNode::Lit)
}

fn variable<'a>() -> impl Parser<&'a str, AstNode, nom::error::Error<&'a str>> {
    map(ident(), AstNode::Var)
}

fn expr<'a>() -> impl Parser<&'a str, AstNode, nom::error::Error<&'a str>> {
    let left = alt((literal(), variable()));
    let op = ws(tag("+"));
    let right = alt((literal(), variable()));
    let add = left.clone().and(op).and(right).map(|((left, _), right)| AstNode::Call {
...
fn func_body<'a>() -> impl Parser<&'a str, Vec<AstNode>, nom::error::Error<&'a str>> {
...
fn parse_func<'a>() -> impl Parser<&'a str, AstNode, nom::error::Error<&'a str>> {
    ...
    fn_kw
        ...
        .map(|((((( _, name), _), _), ret_opt), body)| {
            let ret = ret_opt.map(ToString::to_string).unwrap_or_else(|| "i64".to_string());
            AstNode::FuncDef {
                name,
                generics: vec![],
                params: vec![],
