...
fn main() {
    ...
    println!(
        "  [i32; 10]: {}",
        Type::Array(Box::new(Type::I32), 10).display_name()
    );
    println!(
        "  (i32, bool): {}",
        Type::Tuple(vec![Type::I32, Type::Bool]).display_name()
    );
    ...
    println!(
        "    {} -> {}",
        a.display_name(),
        subst.apply(&a).display_name()
    );
    println!(
        "    {} -> {}",
        b.display_name(),
        subst.apply(&b).display_name()
    );
