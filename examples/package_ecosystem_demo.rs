...
use zetac::package::resolver::{DependencyResolver, PackageMetadata, PackageRegistry};
...
fn demo_manifest_parsing() {
    ...
    manifest.package.description =
        Some("A demonstration library for Zeta package ecosystem".to_string());
...
fn demo_dependency_management() {
    ...
    match resolver.resolve(&manifest) {
        Ok(result) => {
            ...
            println!(
                "Generated lockfile would contain {} entries",
                result.versions.len()
            );
        }
        Err(e) => {
...
fn demo_version_requirements() {
    ...
    let test_cases = vec![
        ...
        (
            ...
            "2.0.0",
            false,
            "Major version bump not compatible (^)",
        ),
