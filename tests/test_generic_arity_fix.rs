...
use zetac::middle::types::{Substitution, Type, TypeVar, GenericContext};
...
fn test_generic_function_arity() {
    ...
    let result = substitution.instantiate_generic_with_bounds(&func_t, &type_args_correct, &context);
    assert!(result.is_ok(), "Should succeed with correct number of type arguments");
    ...
    assert!(result.is_err(), "Should fail with wrong number of type arguments for 2-param function");
    
    // Correct number of type arguments
    let result = substitution.instantiate_generic_with_bounds(&func_t_u, &[Type::I32, Type::Bool], &context);
    assert!(result.is_ok(), "Should succeed with correct number of type arguments for 2-param function");
    
    // Test 3: Named type with type arguments
    let option_t = Type::Named("Option".to_string(), vec![t_var1.clone()]);
    
    // Wrong arity for Option<T>
    let result = substitution.instantiate_generic_with_bounds(&option_t, &[Type::I32, Type::I64], &context);
...
fn test_type_mismatch_errors() {
    ...
    assert!(error_msg.contains("string as integer"), "Error message should mention string/integer mismatch");
    ...
    let error = result.unwrap_err();
    let error_msg = error.to_string();
    assert!(error_msg.contains("boolean as integer"), "Error message should mention boolean/integer mismatch");
    ...
    let error = result.unwrap_err();
    let error_msg = error.to_string();
    assert!(error_msg.contains("integer and floating-point"), "Error message should mention integer/float mismatch");
}