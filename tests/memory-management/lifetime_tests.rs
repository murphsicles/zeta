...
use zetac::middle::types::lifetime::{
    Lifetime, LifetimeContext, LifetimeSubstitution, LifetimeVar,
};
use zetac::middle::types::{Mutability, Type};
...
fn test_lifetime_basics() {
    ...
    assert!(Lifetime::Variable(lv).display_name().starts_with("'L"));
...
fn test_lifetime_in_types() {
    ...
    let static_ref = Type::Ref(Box::new(Type::I32), Lifetime::Static, Mutability::Immutable);

    assert_eq!(static_ref.display_name(), "&'static i32");
    println!(
        "✓ Static lifetime in reference type: {}",
        static_ref.display_name()
    );
    ...
    println!(
        "✓ Named lifetime in reference type: {}",
        named_ref.display_name()
    );
...
fn test_lifetime_elision_patterns() {
    ...
    println!(
        "  ERROR: Cannot infer, must specify: fn longest<'a>(x: &'a str, y: &'a str) -> &'a str"
    );
