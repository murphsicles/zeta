...
fn test_complete_memory_management_workflow() {
    ...
    let static_ref = Type::Ref(Box::new(Type::I32), static_lifetime, Mutability::Immutable);

    let named_ref = Type::Ref(Box::new(Type::Str), named_lifetime, Mutability::Immutable);

    // Declare reference variables
    assert!(
        checker
            .declare("static_ref".to_string(), static_ref)
            .is_ok()
    );
